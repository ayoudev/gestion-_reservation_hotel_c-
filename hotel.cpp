...
int Hotel::CheckOut(int roomno)
{
    ...
    query.prepare("u"
                  "te chamber set available ='y' where number='" +QString::number(roomno)+ "'");
...
int Hotel::BookRoom(int roomno, QString name, QString contactno, QString govtid, QString address)
{
    ...
    query.prepare("update chamber set available ='n' where number='" +QString::number(roomno)+ "'");
    if(!query.exec())
        qDebug() << query.lastError().text() << query.lastQuery();
    else
        qDebug() << "Update was successful "<< query.lastQuery();
    ...
    query.clear();
    query.prepare("insert into client (name, mobileno, govtid, address) values ('" + name + "','" + contactno + "','" + govtid + "','" + address + "')");
    QString customer_id;
    if(!query.exec())
        qDebug() << query.lastError().text() << query.lastQuery();
    else
    ...
    query.clear();
    query.prepare("insert into transactions (room, customer_id) values ('" + QString::number(roomno) + "','" + customer_id + "')");
    if(!query.exec())
        qDebug() << query.lastError().text() << query.lastQuery();
    else
    {
...
std::vector<int> Hotel::getRoomList(QString flag = "y")
{
        ...
        QSqlQuery query(Database);
        query.prepare("select number from chamber where available = '" + flag + "'");
